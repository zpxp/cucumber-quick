...
const workspaceFolder: vscode.Uri | any =
...
export const getCucumberQuickObject = (): CucumberQuickConfiguration => {
  ...
  try {
    let config = vscode.workspace.getConfiguration("cucumber-quick");
    quickConfiguration = {
      tool: config.get("tool") as string,
      script: config.get("script") as string,
    };
  } catch (err) {
    vscode.window.showErrorMessage(
      "unable to read cucumber-quick configuration",
      err
    );
    throw new Error(err);
  }
  ...
  if (quickConfiguration) {
    ...
    vscode.window.showErrorMessage(
      "cucumber-quick configuration not found in .vscode/settings.json"
    );
    throw new Error(
      "cucumber-quick configuration not found in .vscode/settings.json"
    );
 ...
 */
export const getCucumberQuickScript = (
  cucumberQuickConfig: CucumberQuickConfiguration
): string => cucumberQuickConfig.script;

/**
 * get tool information from cucumber.quick configuration
 * @param cucumberQuickConfig
 */
export const getCucumberQuickTool = (
  cucumberQuickConfig: CucumberQuickConfiguration
): string => cucumberQuickConfig.tool;
 ...
 * @param tool
 */
export const executeCucumberQuickCommand = (
  script: string,
  command: string,
  tool?: string
) => {
  ...
  // terminal?.show();
  const executableCommand: string =
    tool === "cucumberjs" ? `${command}` : `${script} ${command}`;
...
const getActiveTerminal = () => {
  return vscode.window.activeTerminal
    ...
    : vscode.window.createTerminal("cucumber-quick");
};
...
export const getScenarioName = () => {
  const selectedLine:
    | string
    | undefined = vscode.window.activeTextEditor?.document.lineAt(
 ...
 * @param scenarioName
 */
export const createCommandToExecuteScenario = (
  scenarioName: string,
  tool: string
): string => {
 ...
 * @param cucumberQuickConfiguration
 */
export const createCommandToExecuteFeature = (
  cucumberQuickConfiguration: CucumberQuickConfiguration
): string => {
  const currentFeatureFilePath: string | undefined =
    vscode.window.activeTextEditor?.document.uri.fsPath;
  const currentRootFolderName:
    | string
    | undefined = vscode.workspace.getWorkspaceFolder(workspaceFolder)?.name;
  ...
  const toolCommands = new Map()
    ...
    .set(
      "cypress",
      `run -e GLOB="${currentFeatureFilePath
        ?.replace(new RegExp(".*" + currentRootFolderName), "")
        .substr(1)}"`
    )
    .set(
      "cucumberjs",
      getCucumberJsFeatureExecutable(
        cucumberQuickConfiguration,
  ...
  if (
    currentFeatureFilePath === undefined &&
    toolCommands.get(cucumberQuickConfiguration.tool) === undefined
  ) {
    vscode.window.showErrorMessage(
